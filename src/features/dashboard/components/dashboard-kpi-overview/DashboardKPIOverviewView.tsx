  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <nav className="inline-flex w-full max-w-fit items-center gap-2 rounded-xl bg-indigo-50 p-1" aria-label="Platform tabs" role="tablist">
          {model.tabs.map((tab) => {
        ...
        {model.onRefresh ? (
          ...
          >
            <RefreshCcw className={`h-4 w-4 ${model.isRefreshing ? 'animate-spin' : ''}`} />
