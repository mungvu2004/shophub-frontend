...
export function AlertsSummaryStrip({
  ...
  return (
    <section className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:flex-row md:items-center md:justify-between">
        ...
        {chips.map((chip) => {
          ...
          return (
            <button
              ...
              className={cn(
                'inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold uppercase tracking-wide ring-1 transition',
                severityColorClass[chip.severity],
                isSelected ? 'scale-[1.02] ring-2 ring-offset-1' : 'opacity-70 hover:opacity-100',
              )}
            >
      ...
      <div className="flex items-center gap-2">
        <Button type="button" variant="ghost" className="h-8 rounded-lg px-3 text-xs font-semibold text-slate-600">
          <Funnel className="size-3.5" />
          Lọc
        </Button>
        <Button type="button" variant="outline" className="h-8 rounded-lg px-3 text-xs" onClick={onClearSeverity}>
